...
class Domains {
    ...
    toDot () {
        ...
        const toStringState = s => {
			...
			for (let i=0; i<domains.data.variables.length; i++) {
				...
				if (states.has(s)) {
					v = getVariableName(domains.data.variables[i], variableNames).replace("'", "");
					break;
				}
			}
    ...
    equalSymbol (variable, symbol) {
        ...
        for (let state of aStates) {
            ...
            for (let s of symbolTos.entries()) {
                if (symbol !== s) {
                    symbolTos.delete(s);
                }
            }
    ...
    union (vd) {
        ...
        const a = new VD(variables, this.fa.minimize());
        const b = new VD(variables, vd.fa.minimize());
        ...
        for (let [from, symbolTos] of a.fa.transitions) {
            for (let tos of symbolTos.values()) {
                for (let to of tos) {
                    a.fa.transition(from, null, to);
                }
            }
        }

        for (let [from, symbolTos] of b.fa.transitions) {
            for (let tos of symbolTos.values()) {
                for (let to of tos) {
                    b.fa.transition(from, null, to);
                }
            }
        }
        ...
        console.log("== a UNION b == | " + variables.join(", "));
        console.log(a.toDot());
        console.log(b.toDot());
        
        const u = a.fa.union(b.fa);
        console.log("----- u ---------");
        console.log(u.toDot());
        ...
        return new VD(variables, u.minimize());
    }
    */
    ...
    union (vd) {
        ...
        for (let i=0; i<variables.length; i++) {
            ...
            const aPosition = a.variables.indexOf(v);
            const bPosition = b.variables.indexOf(v);
