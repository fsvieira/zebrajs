...
class ZVS {
	...
	*/
	...
	merge (
		...
		let branches = [branchsHashs.pop()];

		while (branchsHashs.length) {
			const b = branchsHashs.pop();

			const nr = [];
			for (let i=0; i<branches.length; i++) {
				const a = branches[i];

				const domainBranches = prepareBranches(a, b);
	...
	_merge (
		...
		return this.merge(branchs, prepareBranches, conflictHandler);
	}
