...
async function evalNegation (session, branchId, tupleId) {
	...
	if (data.length) {
		...
		if (h) {
			...
			for (let e of ot.s.values()) {
				console.log("OT => " +  JSON.stringify(e));
			}

			for (let e of nt.s.values()) {
				console.log("NT => " +  JSON.stringify(e));
			}
	...
	try {
		...
		for (let i=0; i<nots.length; i++) {
			...
			if (canEval(zvs, branchId, tupleId, queryVariables)) {
				...
				if (en) {
					...
					if (flop.isEmpty()) {
						console.log("FLOP IS EMPTY!!");
						return;
					}

					for (let v of flop.s.values()) {
		...
		if (flop) {
			...
			if (!count) {
				...
				console.log("E size = " + ene.solution.length);
				for (let i=0; i<ene.solution.length; i++) {
					...
					for (let i=0; i<es.variables.length; i++) {
						...
						if (d.length > 1) {
							...
							valueID = zvs.getObject(branchId, d[0]);
						}
					...
					utils.printQuery(zvs, newBranchId, "NB");
				}
		...
		for (let j = 0; j < bs.length; j++) {
			...
			if (nots && nots.length) {
				...
				if (eBranches && eBranches.length) {
					...
					console.log("Counter=" + tmpCounter);
					utils.printQuery(zvs, branchId, "Branch FAIL");
				}
