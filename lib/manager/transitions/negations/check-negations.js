...
async function checkNegations (action, data, destination, session) {
	...
	branches.forEach(bs => bs.forEach(branchId => utils.checkBranch(zvs, branchId, "CHECK NEGATIONS START")));
	...
	results.forEach(bs => bs.forEach(branchId => utils.checkBranch(zvs, branchId, "CHECK NEGATIONS END")));
