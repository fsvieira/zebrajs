...
function prepareQuery (action, data, destination, session) {
	 ...
	 */
	const variables = getVariables(zvs, queryBranchId, queryId);
	zvs.branches.transform(
		queryBranchId, 
		zvs.data.global("variables"), 
		zvs.data.add({type: 'variables', data: variables.map(v => zvs.getObject(queryBranchId, v))})
	);
