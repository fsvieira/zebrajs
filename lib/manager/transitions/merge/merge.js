...
function mergeDomains (zvs, branchA, branchB) {
	...
	if (domainsA && domainsB) {
			...
			if (b) {
				...
				if (a !== b) {
					...
					if (intersect.size === 1) {
						...
						console.log("Intersect size = " + intersect.size + "; a=" + dataA.data.length + ", b=" + dataB.data.length);
					}
				}
			}
		}
		...
		/*
		const domainsData = [...new Set(
			[...diffA].map(d => zvs.branches.getDataId(branchA, d))
			.concat(
				[...diffB].map(d => zvs.branches.getDataId(branchB, d))
			)
		)].map(v => zvs.getObject(zvs.branches.root, v));

		const allDomainsID = zvs.data.add({
			type: 'domains',
			data: domainsData
		});*/
