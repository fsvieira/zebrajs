...
function mergeDomains (zvs, branchA, branchB) {
	...
	if (domainsA && domainsB) {
		...
		const allDomainsID = zvs.data.add({
			type: 'domains',
			data: [...diffA].map(d => zvs.getObject(branchA, d))
					.concat([...diffB].map(d => zvs.getObject(branchB, d)))
		});
		...
		for (let i=0; i<e.length; i++) {
			...
			zvs.branches.transform(bA, DOMAINS_ID, allDomainsID);
			zvs.branches.transform(bB, DOMAINS_ID, allDomainsID);
