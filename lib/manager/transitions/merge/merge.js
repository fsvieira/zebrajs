...
class ZMerge {
    ...
    tupleXtuple (id, caId, cbId) {
        ...
        if (dataAData.length === dataBData.length) {
            ...
            const negationA = (this.zvs.getData(this.branchA, dataA.negation) || []).map(
                id => this.zvs.getObject(this.branchA, id)
            );

            const negationB = (this.zvs.getData(this.branchB, dataB.negation) || []).map(
                id => this.zvs.getObject(this.branchB, id)
            );
            ...
            const tuple = {
                ...
                check,
                negation: negation.length?negation:undefined
            };
	...
	while (branches.length > 1) {
		...
		for (let i = 0; i < a.length; i++) {
			...
			for (let j = 0; j < b.length; j++) {
                ...
                if (bs && bs.length) {
                    /*
                    bs.forEach(b => utils.printQuery(zvs, b, "BR"));
                    console.log("-- End --");*/
