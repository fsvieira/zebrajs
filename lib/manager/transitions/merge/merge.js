...
function *solveDomains (zvs, branchA, branchB) {
	...
	if (mapAB.size || mapBA.size) {
		...
		// console.log(" -- BUG HERE TO ENE!!-- ");
		const ene = table.toENE();
		// console.log("SoL => " + ene.solution.length);
...
function merge (action, data, destination, session) {
	...
	console.log("--- START ---");
	// branches.forEach((bs, i) => bs.forEach(b => utils.printQuery(zvs, b, "Merge Start [" + i +"]")));
		...
		for (let i = 0; i < a.length; i++) {
			...
			for (let j = 0; j < b.length; j++) {
				...
				for (let [a, b] of solveDomains(zvs, bA, bB)) {
					// utils.printQuery(zvs, a, "a");
					// utils.printQuery(zvs, b, "b");
					...
					if (bs && bs.length) {
						bs.forEach(branchId => utils.printQuery(zvs, branchId, "bA * bB"));
						nr = nr.concat(bs);
					}
				}
			}
	...
	branches.forEach(bs => bs.forEach(b => utils.printQuery(zvs, b, "Merge End")));
	console.log("--- END ---");
