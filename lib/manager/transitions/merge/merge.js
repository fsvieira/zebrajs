...
class MergedBranch {
    ...
    mergeDomains () {
        ...
        if (dA && dB) {
            ...
            console.log("A => " + [...dA.values()].join(", "));
            console.log("B => " + [...dB.values()].join(", "));
            ...
            console.log("aD => " + [...aDiffSet.values()].join(", "));
            console.log("bD => " + [...bDiffSet.values()].join(", "));

            const a = dA.crossProduct(aDiffSet);
            const b = dB.crossProduct(bDiffSet);

            console.log(dA.header, aDiffSet.header);
            console.log(dB.header, bDiffSet.header);

            console.log("A*b => " + [...a.values()].join(", "));
            console.log("B*a => " + [...b.values()].join(", "));


            const ab = a.intersect(b);

            console.log("AB => " + [...ab.values()].join(", "));
    ...
    */
    ...
    branches () {
        if (this.solveConflicts()) {
            ...
            if (rDomains.s.s) {
                // BUG/TODO: is getting to much time on not tests.
                const startTime = new Date().getTime();
                const ene = rDomains.s.toENE().solution;

                stats.time += (new Date()).getTime() - startTime;
                stats.count++;
                console.log(`Time=${(stats.time/(1000*60)).toFixed(3)}m, Count=${stats.count}, Avg=${((stats.time/stats.count)/(1000*60)).toFixed(3)}m`);
