...
class MergedBranch {
    ...
    mergeDomains() {
        console.log("Merge - mergeDomains - Start");
        ...
        if (dA && dB) {
            ...
            console.log("Merge - mergeDomains - End (1)");
            return ab;
        }
        else {
            console.log("Merge - mergeDomains - End (2)");
            return dA || dB;
        }
    }

    solveConflicts() {
        console.log("Merge - solveConflicts - Start");
        ...
        console.log("Merge - solveConflicts - End...");

        return this.mergeConflicts();
    }

    mergeConflicts() {
        console.log("Merge - mergeConflicts - Start");
        while (this.conflicts.length) {
            ...
            if (rID) {
                ...
                // Fail everything
                console.log("Merge - mergeConflicts - End (FALSE)");

                return false;
            }
        }

        console.log("Merge - mergeConflicts - End (TRUE)");
        return true;
    }

    merge(aID, bID) {
        console.log("Merge - merge - Start");
    ...
    */
    ...
    tupleXtuple(a, b) {
        console.log("Merge - tupleXtuple - Start");
        ...
        if (aData.length === bData.length) {
            ...
            for (let i = 0; i < aData.length; i++) {
                ...
                if (id) {
                    ...
                    console.log("Merge - tupleXtuple - End (NULL)");
                    return;
                }
            }
            ...
            console.log("Merge - tupleXtuple - End (id)");
    ...
    */
    variableXvariable(a, b) {
        console.log("Merge - variableXvariable - Start");
        ...
        console.log("Merge - variableXvariable - End");
        return b;
    }

    variableXconstant(a, b) {
        console.log("Merge - variableXconstant - Start");
        a = this.a.rootID(a);

        this.r.transform(a, b);
        console.log("Merge - variableXconstant - End");
        return b;
    }

    variableXdomain(a, b) {
        console.log("Merge - variableXdomain - Start");

        a = this.a.rootID(a);
        b = this.b.rootID(b);

        this.r.transform(a, b);
        console.log("Merge - variableXdomain - End");
        return b;
    }

    variableXtuple(a, b) {
        console.log("Merge - variableXtuple - Start");

        a = this.a.rootID(a);
        b = this.b.rootID(b);

        this.r.transform(a, b);
        console.log("Merge - variableXtuple - End");
        return b;
    }
    ...
    */
    removeDefers(changes) {
        console.log("Merge - removeDefers - Start");
        ...
        console.log("Merge - removeDefers - End");
     ...
     */
    branches() {
        console.log("Merge - branches - Start");

        // if (this.r.domains && this.r.domains.count() === 0) {
        if (this.r.domains && this.r.domains.isEmpty()) {
            console.log("Merge - branches - End (NULL)");

            return;
        }

        if (this.r.domains) {
            console.log("DOMAINS => " + JSON.stringify(this.r.domains.toJSON()));
        }

        console.log("Merge - branches - [DOMAIN IS EMPTY OUT]");

        if (this.solveConflicts()) {
            console.log("Merge - branches - 1");
            const level = Math.max(
                this.zvs.branches.getLevel(this.branchA) + 1,
                this.zvs.branches.getLevel(this.branchB) + 1
            );

            console.log("Merge - branches - 2");
            ...
            console.log("Merge - branches - 4");


            const rawBranch = this.zvs.branches.getRawBranch(branchId);

            console.log("Merge - branches - 5");

            // TODO: should't table be defers free ? 
            this.removeDefers(this.r.table);

            console.log("Merge - branches - 6");

            rawBranch.metadata.changes = this.r.table;

            console.log("Merge - branches - 7");

            rawBranch.domains = rDomains;

            console.log("Merge - branches - End ([branchId])");

            return [branchId];
        }

        console.log("Merge - branches - End (return undefined)");
...
function merge(action, data, destination, session) {
    ...
    while (branches.length > 1) {
        ...
        if (nr.length === 0) {
            ...
            console.log("Merge - merge - End (return!!)");
