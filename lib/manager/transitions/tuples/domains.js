...
function createDomains (zvs, bm, variables, queryBranchId) {
    ...
    if (bm.equals.length) {
        /*
        for (let i=0; i<bm.branches.length; i++) {
            const branchId = bm.branches[i];

            const values = [];
            for (let j=0; j<bm.equals.length; j++) {
                const v = bm.equals[j];
                values.push(zvs.branches.getDataId(branchId, v));
            }
        ...
        console.log("BM => " + JSON.stringify(bm.equals));
        const eq = {};
        ...
        console.log("Equals Resulting branches => " + JSON.stringify(eq));
    ...
    console.log("Diff resulting branches => " + JSON.stringify(df, null, '  '));
    ...
    utils.printQuery(zvs, newBranchId, "New Branch Id");
    ...
    for (let i=0; i<branches.length; i++) {
        ...
        if (variables.length === 0 || bs.length === 1) {
            ...
            for (let i=0; i<bs.length; i++) {
                ...
                console.log("Defer => " + JSON.stringify(deferVars));
                ...
                console.log(utils.toString(zvs.getObject(branchId, tupleId), true));
            }
