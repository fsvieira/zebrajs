...
function *computeBranches (zvs, branches, tupleId, vs, queryBranchId) {
    ...
    for (let i=0; i<ene.solution.length; i++) {
        ...
        for (let i=0; i<es.variables.length; i++) {
            ...
            if (d.length > 1) {
                // value is a domain,
                const dData = zvs.getData(queryBranchId, vID);
                const dID = zvs.getData(queryBranchId, dData.id);
                const domain = {
                    type: "domain",
                    data: d.map(v => zvs.getObject(queryBranchId, v)),
                    id: dID
                ...
                valueID = zvs.data.add(zvs.getObject(queryBranchId, d[0]));
            }
...
function domains (action, data, destination, session) {
    ...
    branches.forEach(({branches: bs}) => bs.forEach(branchId => utils.checkBranch(zvs, branchId, "DOMAIN CHECK START")));
    ...
    results.forEach(bs => bs.forEach(branchId => utils.checkBranch(zvs, branchId, "DOMAIN CHECK END")));
