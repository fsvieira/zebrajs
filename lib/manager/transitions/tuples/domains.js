...
function *computeBranches (zvs, branches, tupleId, vs, queryBranchId) {
    ...
    for (let branchId of branches) {
        const domains = header.map(id => {
            ...
            const v = zvs.getObject(queryBranchId, id);
            const dID = zvs.data.add({type: 'domain', data: v.data, id: v.id});

            zvs.branches.transform(newBranchId, id, dID);

            if (type === 'constant') {
                return [v.id, vID];
            }
            else {
                return [v.id, zvs.getData(branchId, data.data)];
            }
        });

        // 4. add values to table.
        let r = zvs.branches.getDomains(branchId);

        for (let i=0; i<domains.length; i++) {
            const [h, v] = domains[i];
            const rs = new CSetArray([v]).as(h);

            r = r?r.crossProduct(rs):rs;
        }
    ...
    zvs.branches.setDomains(newBranchId, s);
