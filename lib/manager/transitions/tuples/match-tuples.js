...
function matchTuples (action, data, destination, session) {
	...
	if (tuplesDefinitions) {
		// utils.printQuery(zvs, branchId, "MATCH");

		/*
		tuplesDefinitions.map( ({tuple, definitions}, index) => console.log(index + " : " +
			utils.toString(zvs.getObject(branchId, tuple), true) + " ** [" +
			definitions.map(d => utils.toString(zvs.getObject(branchId, d))).join(" | ") + "]"
		));*/	
