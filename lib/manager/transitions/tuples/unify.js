...
function constantXdomain (zvs, branchId, p, q) {
	...
	// since domains contain original variables ids we need to search for updated p value.
	domainsData.splice(domainsData.map(v => zvs.branches.getDataId(branchId, v)).indexOf(q), 1);
	zvs.branches.transform(branchId, q, p);

	const remove = new Set([p]);

	for (let e of remove) {
		for (let i=domainsData.length-1; i>=0; i--) {
	...
	const newDomainsID = zvs.data.add({
		...
		// keep original ids ? 
		data: domainsData.map(v => {
			const o = zvs.getObject(zvs.branches.root, v);

			console.log("Save to domain => " + JSON.stringify(o));
			return o;
		}),
		id: [branchId]
	});
