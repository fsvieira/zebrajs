...
function tupleXvariable (zvs, branchId, p, q) {
	variableXtuple(zvs, branchId, q, p);
	return true;
}
...
function _domainXdomain (zvs, branchId, p, q) {
...
function constantXdomain (zvs, branchId, p, q) {
	...
	for (let e of remove) {
		for (let i=0; i<domainsData.length; i++) {
			...
			const data = zvs.getData(domain.data);
	...
	zvs.update(
		...
		DOMAINS_ID,
		{
			data: domainsData.map(v => zvs.getObject(v))
		}
	);
...
function _constantXdomain (zvs, branchId, p, q) {
	const domain = zvs.getData(branchId, q);
	const values = zvs.getData(branchId, domain.data);
	// const {data: variables} = zvs.getData(branchId, zvs.data.global("variables"));
	const data = zvs.getData(branchId, zvs.data.global("variables"));
	const variables = zvs.getData(branchId, data.data);
	/*
	for (let i=0; i<variables.length; i++) {
		const v = variables[i];

		const domain = zvs.getData(branchId, v);
		console.log(JSON.stringify(domain));
	}*/

	if (values.includes(p)) {
		zvs.branches.transform(branchId, q, p);
		variables.splice(variables.indexOf(domain.id), 1);

		// forall variables remove constant,
		for (let i=0; i<variables.length; i++) {
			const v = variables[i];
			const d = zvs.getData(branchId, v);
			...
			if (index !== -1) {
				...
				if (data.length > 1) {
					zvs.update(branchId, v, {data: data.map(v => zvs.getObject(branchId, v))});
				}
				else if (data.length === 1) {
					console.log("TODO: make this constant, remove constant from all other domains!!");
				}
				else {
					return false;
				}
	...
	console.log(JSON.stringify(po) + " ** " + JSON.stringify(qo));
...
function _domainsXdomains (zvs, branchId, p, q) {
	const po = zvs.getObject(branchId, p);
	const qo = zvs.getObject(branchId, q);
...
function variablesXvariables (zvs, branchId, p, q) {
	const {data: aID} = zvs.getData(branchId, p);
	const {data: bID} = zvs.getData(branchId, q);

	const a = zvs.getData(branchId, aID).map(v => zvs.branches.getDataId(branchId, v));
	const b = zvs.getData(branchId, bID).map(v => zvs.branches.getDataId(branchId, v));

	const vs = [...(new Set([...a, ...b]))];

	const variables = vs.map(v => zvs.getObject(branchId, v))
		// TODO: variables should only contain variables or domains, and they should have an id or ids.
		.filter(v => v.type === 'variable')
	;

	variables.sort((a, b) => a.id.localeCompare(b.id));

	const variablesID = zvs.data.add({
		type: 'variables',
		data: variables
	});

	zvs.branches.transform(branchId, p, variablesID);
	zvs.branches.transform(branchId, q, variablesID);
