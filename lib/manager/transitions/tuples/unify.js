...
function unify (zvs, branchId, p, q, evalNegation) {
	/*const {end, proxy} = utils.recorder("unify", zvs);
	zvs = proxy;
	*/
	...
	console.log(
		...
		utils.toString(zvs.getObject(branchId, q)) + " [" + p + ", " + q + "]\n"
	);
	*/
	...
	if (!r || !update(zvs, branchId, p, q)) {
		...
		/*
		console.log(
			utils.toString(zvs.getObject(branchId, p), true) + " ** " +
			utils.toString(zvs.getObject(branchId, q), true) + " ==> FAIL"
		);
	...
	/*
	console.log(
		utils.toString(zvs.getObject(branchId, p), true) + " ["+p+"] ** " +
		utils.toString(zvs.getObject(branchId, q), true) + " ["+q+"] ==> OK"
	);

	console.log(
		JSON.stringify(zvs.getObject(branchId, p)) + " ** " +
		JSON.stringify(zvs.getObject(branchId, q)) + " ==> OK"
	);

	utils.printQuery(zvs, branchId, "UNIFY R");
	*/
