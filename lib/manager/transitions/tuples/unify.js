...
function unify (zvs, branchId, p, q, evalNegation) {
	...
	const debugPQ = utils.toString(zvs.getObject(branchId, p), true) + " ** " + utils.toString(zvs.getObject(branchId, q), true);
	...
	if (!r || !update(zvs, branchId, p, q)) {
		...
		console.log(debugPQ + " => false");
		return;
	}

	console.log(debugPQ + " => " + branchId);
